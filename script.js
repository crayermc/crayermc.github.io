...
/* Store */
let cart=JSON.parse(localStorage.getItem("crayerCart")||"[]");
function renderCart(){
...
function saveCart(){localStorage.setItem("crayerCart",JSON.stringify(cart));renderCart();}
document.querySelectorAll(".add-cart").forEach(btn=>btn.addEventListener("click",()=>{
  const card=btn.closest(".product-card");
  cart.push({name:card.dataset.name,price:Number(card.dataset.price),priceLabel:card.querySelector("strong").textContent});
...
if(order)order.onclick=async()=>{
  ...
  const message=`CRAYER MC STORE ORDER\n\n${lines}\n\nTOTAL: ${total}\n\nMinecraft IGN: [YOUR IGN]`;
